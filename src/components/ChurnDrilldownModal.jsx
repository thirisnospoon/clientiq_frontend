...
/* Fake snapshot for CHURN — seed includes metric */
function makeChurnSnapshot(date) {
    const key = `${dayjs(date).format("YYYY-MM-DD")}|churn`;
...
export default function ChurnDrilldownModal({ open, onClose }) {
    ...
    const snapA = useMemo(() => makeChurnSnapshot(primaryDate), [primaryDate]);
    const snapB = useMemo(() => compareDate ? makeChurnSnapshot(compareDate) : null, [compareDate]);
    ...
    const colorA = theme.palette.warning.main;     // Churn color
    ...
    return (
        <Dialog fullScreen open={open} onClose={onClose} TransitionComponent={Transition} BackdropProps={{ sx: { backdropFilter: 'blur(3px)' } }}>
            <AppBar position="relative" elevation={0} sx={{
                ...
                <Toolbar>
                    <Typography sx={{ flex: 1, fontWeight: 700, letterSpacing: 0.2 }} variant="h6">Churn Risk Drilldown</Typography>
                ...
                <Box sx={{ maxWidth: 1200, mx: 'auto', display: 'grid', gap: isMobile ? 2 : 3 }}>
                    ...
                    <Paper elevation={0} sx={{ p: isMobile ? 2 : 3, borderRadius: 3, border: '1px solid', borderColor: 'divider', bgcolor: theme.palette.background.paper }}>
                        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5 }}>
                            <Stack direction="row" spacing={1.25} alignItems="center">
                                <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Churn Risk by Mark (1–10)</Typography>
                        ...
                        <Box sx={{ width: "100%", height: isMobile ? 300 : 380 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={mergedChartData} margin={{ top: 10, right: 20, bottom: 5, left: 0 }}>
                                    ...
                                    <Legend />
                                    <Bar dataKey="A" name={`A (${snapA.date})`} fill="url(#barA-churn)" radius={[8, 8, 0, 0]} />
                                    {snapB && <Bar dataKey="B" name={`B (${snapB.date})`} fill="url(#barB-churn)" radius={[8, 8, 0, 0]} />}
                                </BarChart>
                            </ResponsiveContainer>
                        </Box>
                    </Paper>

...
function makeEngageSnapshot(date) {
...
/* ---------- Custom tooltip ---------- */
function CustomTooltip({ active, payload, label, theme, snapA, snapB }) {
    if (!active || !payload?.length) return null;

    const colorA = theme.palette.info.main;
            ...
            <Stack spacing={0.75}>
                ...
                <Stack direction="row" spacing={1.25} alignItems="center">
                    ...
                    <Typography variant="body2">
                        A <span style={{ opacity: 0.6 }}>({snapA.date})</span>: <b>{a?.toLocaleString?.() ?? '—'}</b>
                    </Typography>
                </Stack>
                ...
                {snapB && (
                    <Stack direction="row" spacing={1.25} alignItems="center">
                        ...
                        <Typography variant="body2">
                            B <span style={{ opacity: 0.6 }}>({snapB.date})</span>: <b>{b?.toLocaleString?.() ?? '—'}</b>
                        </Typography>
                    </Stack>
                )}
...
export default function EngagementDrilldownModal({ open, onClose }) {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down("sm"));

    const [primaryDate, setPrimaryDate] = useState(dayjs());
    const [compareDate, setCompareDate] = useState(null); // B is shown only if not null

    const snapA = useMemo(() => makeEngageSnapshot(primaryDate), [primaryDate]);
    const snapB = useMemo(() => compareDate ? makeEngageSnapshot(compareDate) : null, [compareDate]);
    ...
    const totalsDelta = useMemo(() => {
        ...
        return formatDelta(snapA.total, snapB.total);
    }, [snapA, snapB]);
    ...
    const resetBoth = () => {
        ...
        setCompareDate(null); // hide B completely
    };
    ...
    return (
        ...
        >
            ...
            >
                <Toolbar>
                    <Typography sx={{ flex: 1, fontWeight: 700, letterSpacing: 0.2 }} variant="h6" component="div">
                            ...
                            <Grid container spacing={2} alignItems="center">
                                <Grid item xs={12} md={4}>
                                    ...
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        <DatePicker
                                            value={primaryDate}
                                            onChange={(val) => setPrimaryDate(val ?? dayjs())}
                                            disableFuture
                                            slotProps={{ textField: { size: "small", fullWidth: true } }}
                                        />
                                        <Tooltip title="Reset to today & clear comparison">
                                            <IconButton size="small" onClick={resetBoth}>
                                ...
                                <Grid item xs={12} md={4}>
                                    <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: isMobile ? 1 : 4 }}>
                                        ...
                                        {/* B chip only if compare date set */}
                            ...
                            <Grid item xs={12} md={6}>
                                ...
                                >
                                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={2}>
                                        <Box>
                                            <Typography variant="subtitle2" sx={{ opacity: 0.75 }}>
                                                Total Active Clients (B)
                                            </Typography>
                                            <Typography variant={isMobile ? "h4" : "h3"} sx={{ fontWeight: 800, lineHeight: 1.2 }}>
                                                {snapB.total.toLocaleString()}
                                            </Typography>
                                            ...
                                            {totalsDelta && (
                                                <Stack direction="row" spacing={1} alignItems="center" justifyContent="flex-end">
                                                    ...
                                                    <Typography variant="body1" sx={{ fontWeight: 700 }}>
                                                        {totalsDelta.diff >= 0 ? "+" : ""}{totalsDelta.diff.toLocaleString()}
                                                    </Typography>
                                                    <Typography variant="body2" sx={{ opacity: 0.75 }}>
                                                        ({totalsDelta.pct >= 0 ? "+" : ""}{totalsDelta.pct.toFixed(1)}%)
                                                    </Typography>
                                                </Stack>
                                            )}
                                        </Box>
                        ...
                        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5 }}>
                            ...
                            <Stack direction="row" spacing={1}>
                                <Chip
                                    size="small"
                                    label={`A: ${snapA.date}`}
                                    sx={{ bgcolor: alpha(colorA, 0.12), color: colorA, borderColor: alpha(colorA, 0.3), border: '1px solid' }}
                                />
                                {snapB && (
                                    <Chip
                                        size="small"
                                        label={`B: ${snapB.date}`}
                                        sx={{ bgcolor: alpha(colorB, 0.12), color: colorB, borderColor: alpha(colorB, 0.3), border: '1px solid' }}
                                    />
                                )}
                            </Stack>
                        </Stack>
                        ...
                        <Box sx={{ width: "100%", height: isMobile ? 300 : 380 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={mergedChartData} margin={{ top: 10, right: 20, bottom: 5, left: 0 }}>
                                    ...
                                    <RTooltip content={(props) => <CustomTooltip {...props} theme={theme} snapA={snapA} snapB={snapB} />} />
                                    <Legend />
                                    <Bar dataKey="A" name={`A (${snapA.date})`} fill="url(#barA)" radius={[8,8,0,0]} />
                                    {snapB && <Bar dataKey="B" name={`B (${snapB.date})`} fill="url(#barB)" radius={[8,8,0,0]} />}
                                </BarChart>
                            </ResponsiveContainer>
                        </Box>
                    </Paper>
